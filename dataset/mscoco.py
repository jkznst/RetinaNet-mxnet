...
class Coco2017(Imdb):
    ...
    """
    def __init__(self, anno_file, image_set, image_dir, shuffle=True, names='mscoco.names'):
        assert os.path.isfile(anno_file), "Invalid annotation file: " + anno_file
        basename = os.path.splitext(os.path.basename(anno_file))[0]
        super(Coco2017, self).__init__('coco_' + basename)
        self.image_dir = image_dir
    ...
    def _load_all(self, anno_file, shuffle):
        ...
        class_to_coco_ind = dict(zip(cats, _coco.getCatIds()))    # 1 to 90 with 10 classes missed
        class_to_ind = dict(zip(self.classes, range(self.num_classes)))  # 0 to 79
        coco_ind_to_class_ind = dict([(class_to_coco_ind[cls], class_to_ind[cls]) for cls in self.classes])

        img_ids = _coco.getImgIds()
        for img_id in img_ids:
            # filename
            image_info = _coco.loadImgs(img_id)[0]
            filename = image_info["file_name"]
            # subdir = filename.split('_')[1]
            subdir = self.image_set
            height = image_info["height"]
            width = image_info["width"]
            # label
            anno_ids = _coco.getAnnIds(imgIds=img_id)
            annos = _coco.loadAnns(anno_ids)
            label = []
            for anno in annos:
                cat_id = int(anno["category_id"])
                bbox = anno["bbox"]
                ...
                xmax = xmin + float(bbox[2]) / width
                ymax = ymin + float(bbox[3]) / height
                label.append([coco_ind_to_class_ind[cat_id], xmin, ymin, xmax, ymax, 0])
            if label:
                labels.append(np.array(label))
                image_set_index.append(os.path.join(subdir, filename))
